...
mat_4D matrix_add(mat_4D a, mat_4D b)
{
	mat_4D res;
	for(int i = 0; i < 4; i++){
		for(int j = 0; j < 4; j++){
			res.m[4*i+j] = a.m[4*i+j] + b.m[4*i+j];
		}
	}
...
mat_4D shear(float sx, float sy, float sz)
{
	mat_4D res = identity_matrix();
	res.m[0] = sx;
	res.m[5] = sy;
...
mat_4D rot(float xy, float xz, float yz)
{
	mat_4D res = zero_matrix();
